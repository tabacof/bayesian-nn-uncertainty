...
X_train, y_train, X_test, y_test, X_test_all, y_test_all = datasets.load_CIFAR10(range(10))
...
if train_weights:
    ...
    np.savez('cifar_dropout_convnet.npz', *lasagne.layers.get_all_param_values(network))
else:
    with np.load('cifar_dropout_convnet.npz') as f:
        param_values = [f['arr_%d' % i] for i in range(len(f.files))]
    lasagne.layers.set_all_param_values(network, param_values) 
        
mean_cifar = 0.4733623
